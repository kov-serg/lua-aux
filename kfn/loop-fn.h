...
/* for manual labeling: enum { L01=-100,L02,L03,L04 }; ... LOOP_POINT_(L02) */
#define LOOP_POINT_(name) { *__loop=name; goto __loop_leave; case name:{} }
#define LOOP_INT_(name,n) { __loop_rv=(n); LOOP_POINT_(name) }
/* extension for preudo asm */
#define LOOP_TERM()       { goto __loop_end; }
#define LOOP_LABEL(name)  { case name:{} }
#define LOOP_GOTO(name)   { *__loop=name; goto __loop_leave; }
#define LOOP_JUMP(name)   { *__loop=name; goto __loop_switch; }
